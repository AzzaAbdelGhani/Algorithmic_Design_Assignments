...
void sub_matrix_blocks(float **C, float const *const *const A,
                  ...
                  const size_t n) 
{
    for (size_t y = 0; y < n; y++){ // y is the row in A
 ...
 * respectively, of the sub-matrix we want to deal with.
 */
//we are going to write an auxiliary function to ease the API of our code
...
void strassen_aux(float **C, float const *const *const A,
                  ...
                  const size_t A_f_row, const size_t A_f_col,
                  const size_t B_f_row, const size_t B_f_col,
                  const size_t n) // here we will pass the index of top left element of each block and its size will be half of n
{
    if (n <= 128) //when n was 1 , the naive algo was better in some instances than strassen
                  // we increase n and strassen is much much better now
    {
        naive_aux(C,A,B,
                  C_f_row,C_f_col,
                  A_f_row,A_f_col,
    ...
    //S1 = B12 - B22
    sub_matrix_blocks(S[0], B, B, 0, 0, B_f_row, B_f_col+n2,
                      B_f_row + n2, B_f_col + n2, n2);
